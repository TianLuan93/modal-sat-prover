...
def modal_sat_main(X0):
    ...
    s0 = Minisat22()
    for phi in X0:
        if "[]" not in phi.cluaseInModalContent \
        ...
        for k0 in X:
            if "<>" in k0.cluaseInModalContent and \
                ...
                if r1:
                    phi = []
                    for k1 in X:
                        if ("[]" in k1.cluaseInModalContent or "<>" in k1.cluaseInModalContent) and k1.boxDepth == box_depth:
                            if "[]" in k1.cluaseInModalContent:
                                operator = "[]"
                            else:
                                operator = "<>"
                            z = convertToInputMain(k1.cluaseInModalContent[1:k1.cluaseInModalContent.index(operator)])[0]
